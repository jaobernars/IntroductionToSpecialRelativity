document.addEventListener('DOMContentLoaded', () => {
    ...
    if (toggleBtn && sidebar) {
        toggleBtn.addEventListener('click', () => {
            ...
            sidebar.classList.toggle('active'); // Added to trigger mobile visibility
        });
    }
    ...
    if (themeToggleBtn) {
        // Tenta obter o tema salvo ou usa "dark" por padrão
    ...
    window.addEventListener('scroll', () => {
        ...
        sections.forEach(section => {
            ...
            // Compensando a altura desejada (aprox. 300px do topo)
    ...
    // Executa uma vez no carregamento para checar o item inicial
    window.dispatchEvent(new Event('scroll'));

    // --- Funcionalidade de Pesquisa Simples (UL) ---
    ...
    if (searchTrigger && searchOverlay && searchInput && searchResults) {
        // Criar uma lista simples estruturada apenas uma vez
        ...
        sidebarGroups.forEach(group => {
            ...
            links.forEach(link => {
                ...
                a.href = link.href; // Navegação 100% nativa do HTML
                a.className = 'search-item'; // Mantém o estilo CSS padrão do hover
        ...
        // Adiciona a lista ao DOM (feita apenas uma vez)
        ...
        searchInput.addEventListener('keydown', (e) => {
            if (e.key === 'ArrowDown') {
                ...
                if (selectedIndex >= 0 && selectedIndex < visibleItems.length) {
                    // Clica fisicamente no link
                    visibleItems[selectedIndex].aElement.click();
                }
            }
        });

        // Fecha o modal se clicar num link (importante para âncoras na mesma página)
    ...
    // Breadcrumbs dinâmicos (Left Sidebar)
    ...
    // Atualiza automaticamente no carregamento da página com base no item ativo
    updateBreadcrumbs();
});
